...
const getRolesForUpdate = async (req, res, next) => {
  try {
    if (!req.user || !req.user.role) {
      return res.status(403).json({
        success: false,
...
router.put('/:id', AuthMiddleware.verifyAuth, getRolesForUpdate, controller.updatePatient.bind(controller));
